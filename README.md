...
# Part 2
Considering handling large files (~100mb) with high rates - this will require split storage from the database. This approach ensures the SQL database remains fast for metadata storage, while a highly scalable Object Storage system manages the large files.
...
Instead client code sending file to Web API, client code sends a request to the API ```(POST /api/submissions/upload-url)``` requesting permission to upload a specific file (identified by its expected size/type).
 ```
{
  "submissionId": "...",
...
```
{
  "storageKey": "submissions/89d7b42a/proposal_1.docx",
  "uploadUrl": "[https://tinystorage.s3.amazonaws.com/](https://tinystorage.s3.amazonaws.com/)...", // Temporary, secure URL
  "expiryTime": "2025-11-17T10:30:00Z"
}
``` 
